...
if uploaded_file:
    ...
    speed_min = st.sidebar.number_input("Min Speed (RPM)", value=0.0)
    speed_max = st.sidebar.number_input("Max Speed (RPM)", value=1e6)
    dbse_input = st.sidebar.number_input("DBSE /DBFF (mm)", value=0.0, help="Optional: Searches ±10mm if value provided")
    ...
    if st.sidebar.button("🔍 Search Couplings"):
        ...
        output_columns = [
            ...
            "torsional stiffness (mnm/rad)", "dbse /dbff (mm)", "total weight\n(kg)"
        ]
